...
export const QRProvider = ({ children }) => {
  ...
  const generateQRCode = async (bookingId) => {
    try {
      ...
      const response = await fetch(`http://localhost:3001/api/qr/generate/${bookingId}`, {
        method: 'POST',
        headers: {
  ...
  const getQRCode = async (bookingId) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/qr/booking/${bookingId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const verifyQRCode = async (qrCodeString) => {
    try {
      ...
      const response = await fetch('http://localhost:3001/api/qr/verify', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const processCheckIn = async (qrCodeString, location = null) => {
    try {
      ...
      const response = await fetch('http://localhost:3001/api/qr/checkin', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const processCheckOut = async (bookingId, notes = '', rating = null) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:3001/api/qr/checkout', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
  ...
  const getCheckInStatus = async (bookingId) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/qr/status/${bookingId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const getAttendanceHistory = async (limit = 10, skip = 0) => {
    try {
      ...
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/qr/attendance?limit=${limit}&skip=${skip}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const value = {
    ...
    sendQREmail: async (bookingId, userEmail, userData) => {
      try {
        ...
        const token = localStorage.getItem('token');
        const response = await fetch(`http://localhost:3001/api/qr/email/${bookingId}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json'
