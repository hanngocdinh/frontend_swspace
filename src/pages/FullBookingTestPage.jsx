...
const FullBookingTestPage = () => {
  ...
  const testFullBookingFlow = async () => {
    ...
    try {
      ...
      const beforeResponse = await fetch('http://localhost:3001/api/bookings/seats/occupied?serviceType=hot-desk&date=2025-10-27', {
      ...
      const updateResponse = await fetch('http://localhost:3001/api/bookings/' + bookingResult.booking.id + '/confirm-payment', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const afterResponse = await fetch('http://localhost:3001/api/bookings/seats/occupied?serviceType=hot-desk&date=2025-10-27', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      ...
      const historyResponse = await fetch('http://localhost:3001/api/bookings', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
