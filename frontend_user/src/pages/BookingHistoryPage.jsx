...
const BookingHistoryPage = () => {
  ...
  const filterBookings = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(booking =>
        ...
        booking.seatName.toLowerCase().includes(searchTerm.toLowerCase())
      );
    }
  ...
  const handleDeleteBooking = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
  const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:5000';
      const response = await fetch(`${apiUrl}/api/bookings/${deleteModal.booking._id}/permanent`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (data.success) {
        ...
        setBookings(prev => prev.filter(booking => booking._id !== deleteModal.booking._id));
                    ...
                    <BookingInfo>
                      ...
                      <InfoSection>
                        ...
                        <InfoDetails>
                          ...
                          <div><strong>Payment:</strong> {booking.paymentStatus || 'Pending'}</div>
                        </InfoDetails>
                      </InfoSection>
                    </BookingInfo>
